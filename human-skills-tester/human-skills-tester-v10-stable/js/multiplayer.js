...
async function watchRoom(id){
  ...
  await attachConnection(id);await set(ref(rtdb,`socialPresence/${user.uid}`),{state:'online',activity:'multiplayer',roomId:id,gameType:(await get(ref(rtdb,`rooms/${id}/type`))).val(),rounds:(await get(ref(rtdb,`rooms/${id}/rounds`))).val()||1,joinable:true,lastSeen:rtdbTimestamp()});
...
async function renderRoom(room){
  ...
  if(room.status==='countdown'||room.status==='playing'||room.status==='betweenRounds'){localStorage.setItem('hst_active_room',room.id);await update(ref(rtdb,`socialPresence/${user.uid}`),{activity:'multiplayer',roomId:room.id,gameType:room.type,rounds:room.rounds||1,joinable:false,lastSeen:rtdbTimestamp()});location.replace(`battle.html?room=${room.id}`)}
}
