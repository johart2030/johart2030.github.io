...
async function boot(){
  ...
  const playerRef=ref(rtdb,`rooms/${roomId}/players/${user.uid}`);await onDisconnect(playerRef).update({connected:false,lastSeen:rtdbTimestamp()});await update(playerRef,{connected:true,lastSeen:rtdbTimestamp()});await set(ref(rtdb,`socialPresence/${user.uid}`),{state:'online',activity:'multiplayer',roomId,gameType:room.type,rounds:room.rounds||1,joinable:false,lastSeen:rtdbTimestamp()});
