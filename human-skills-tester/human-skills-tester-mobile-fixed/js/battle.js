...
async function ensureMembership(){
  ...
  const tx=await runRTDBTransaction(ref(rtdb,`rooms/${roomId}`),r=>{
    if(!r)return;const players=r.players||{};if(players[user.uid])return r;if(Object.keys(players).length>=2)return;
...
async function boot(){
  ...
  user=await waitForAuth();if(!user){sessionStorage.setItem('hst_after_auth',location.href);return location.replace(`login.html?next=${encodeURIComponent('battle.html?room='+roomId)}`)}
