...
async function requirePlayer(){
  ...
  if(!user){saveDestination(location.href);location.href=`login.html?next=${encodeURIComponent(location.pathname.split('/').pop()+location.search)}`;return false}
...
async function joinRoom(rawId){
  ...
  if(!result.committed){const check=await get(ref(rtdb,`rooms/${id}`));if(!check.exists())throw new Error('Room not found or expired.');const r=check.val();if(r.status!=='waiting')throw new Error('This match has already started.');throw new Error('This room is full.');}
