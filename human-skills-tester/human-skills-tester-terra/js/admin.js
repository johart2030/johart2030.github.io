...
async function resolveLogOwners() {
  ...
  const resolved = await Promise.all(missingIds.map(async uid => {
    try { return [uid, await profileFor(uid)]; } catch { return [uid, null]; }
  }));
...
async function inspectUser(user) {
  ...
  try {
    ...
    userData.textContent = JSON.stringify({ profile: fields, scores: Object.fromEntries(scores.docs.map(score => [score.id, score.data().value])), recentTabSwitchLogs: userLogs }, null, 2);
...
async function loadAdminData() {
  ...
  try {
    ...
    users = userSnapshot.docs.map(snapshot => ({ id: snapshot.id, ...snapshot.data() })).sort((a, b) => labelFor(a).localeCompare(labelFor(b)));
