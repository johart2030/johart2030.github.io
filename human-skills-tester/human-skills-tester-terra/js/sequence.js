...
function showBest() {
    best.textContent = HST.get('sequence', 0);
}
async function show() {
    accepting = false;
    msg.textContent = `Level ${sequence.length}: watch`;
    await sleep(500);
    for (const pad of sequence) {
        pads[pad].classList.add('flash');
        await sleep(420);
        pads[pad].classList.remove('flash');
        await sleep(180);
    }
    position = 0;
    accepting = true;
...
pads.forEach((pad, index) => pad.onclick = () => {
    if (!accepting)
        return;
    ...
    if (index !== sequence[position]) {
        accepting = false;
        msg.textContent = `Wrong pad. You reached level ${sequence.length}.`;
    ...
    if (position === sequence.length) {
        accepting = false;
        HST.setBest('sequence', sequence.length);
