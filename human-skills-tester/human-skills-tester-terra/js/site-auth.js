...
export const ready = new Promise(resolve => onAuthStateChanged(auth, async user => {
  if (!user && !resolved) {
    resolved = true;
    try { await signInAnonymously(auth); return; } catch (error) { console.warn('Anonymous auth unavailable', error); }
  }
  resolved = true;
  ...
  if (user && !user.isAnonymous) {
    try {
      ...
      if (profile && !profile.role) {
        await setDoc(doc(db, 'users', user.uid), { role: 'user', updatedAt: serverTimestamp() }, { merge: true });
        profile = { ...profile, role: 'user' };
      }
      if (profile?.displayName) await ensurePublicProfile(user, profile);
    } catch (error) { console.warn('Profile migration failed', error); }
  }
  window.hstProfile = profile;
  paint(user, profile);
  if (user) setupPresence(user, profile);
...
function paint(user, profile) {
  ...
  document.querySelectorAll('[data-signout]').forEach(button => button.onclick = async () => { await signOut(auth); location.href = 'index.html'; });
}
...
function setupPresence(user, profile) {
  ...
  onValue(connected, async snapshot => {
    ...
    try {
      ...
      await set(session, { state: 'online', signedIn: !user.isAnonymous, displayName: user.isAnonymous ? 'Guest' : profile?.displayName || user.displayName || 'Player', lastChanged: rtdbTimestamp() });
      if (!user.isAnonymous) {
        const social = ref(rtdb, `socialPresence/${user.uid}`);
        await onDisconnect(social).set({ state: 'offline', lastSeen: rtdbTimestamp() });
        await set(social, { state: 'online', activity: 'browsing', joinable: false, lastSeen: rtdbTimestamp() });
      }
    } catch (error) { console.warn('Presence unavailable', error); }
  });
  onValue(ref(rtdb, 'activeSessions'), snapshot => {
    let count = 0;
    snapshot.forEach(uid => { if (uid.exists()) count += 1; });
