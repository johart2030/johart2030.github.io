...
function selectedRounds() {
    const value = Number($('#roundCount')?.value || 1);
    return [1, 3, 5, 7, 10].includes(value) ? value : 1;
}
...
function newPlayer(existing = {}) {
    return {
        ...
        connected: true,
        joinedAt: existing.joinedAt ?? rtdbTimestamp(),
...
async function createRoom(type = 'reaction', isPublic = false) {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
        ...
        if (result.committed) {
            roomId = id;
            localStorage.setItem('hst_active_room', id);
            if (isPublic)
                await set(ref(rtdb, `openRooms/${id}`), {
                    type,
                    rounds,
...
async function joinRoom(rawId) {
    ...
    if (!verify.exists() || !verify.val().players?.[user.uid])
        throw new Error('Your player slot could not be created.');
    roomId = id;
    localStorage.setItem('hst_active_room', id);
    await remove(ref(rtdb, `openRooms/${id}`)).catch(() => {
    });
    await watchRoom(id);
}
async function attachConnection(id) {
    ...
    if (connectionUnsub)
        connectionUnsub();
    connectionUnsub = onValue(ref(rtdb, '.info/connected'), snapshot => {
        const badge = $('#connectionStatus');
        if (badge) {
            badge.textContent = snapshot.val() ? 'Connected' : 'Reconnecting…';
            badge.className = `connection-badge ${snapshot.val() ? 'online' : 'offline'}`;
        }
    });
}
async function watchRoom(id) {
    ...
    showMessage('Waiting for everyone to be ready.');
    await attachConnection(id);
    await set(ref(rtdb, `socialPresence/${user.uid}`), {
        state: 'online',
        activity: 'multiplayer',
        roomId: id,
        gameType: (await get(ref(rtdb, `rooms/${id}/type`))).val(),
        rounds: (await get(ref(rtdb, `rooms/${id}/rounds`))).val() || 1,
        joinable: true,
        lastSeen: rtdbTimestamp()
    });
    if (roomUnsub)
        roomUnsub();
    roomUnsub = onValue(ref(rtdb, `rooms/${id}`), snapshot => {
        if (!snapshot.exists()) {
            showMessage('This room was closed.', 'error');
            return showLobby();
        }
        renderRoom(snapshot.val()).catch(error => showMessage(error.message, 'error'));
    }, error => showMessage(`Room connection failed: ${error.message}`, 'error'));
}
async function renderRoom(room) {
    ...
    if (room.status === 'waiting' && room.host === user.uid && players.length >= 2 && players.every(([, p]) => p.ready) && !startLock) {
        ...
        const transaction = await runRTDBTransaction(ref(rtdb, `rooms/${room.id}`), current => {
            ...
            if (currentPlayers.length < 2 || !currentPlayers.every(p => p.ready))
                return;
    ...
    if (room.status === 'countdown' || room.status === 'playing' || room.status === 'betweenRounds') {
        localStorage.setItem('hst_active_room', room.id);
        await update(ref(rtdb, `socialPresence/${user.uid}`), {
            activity: 'multiplayer',
            roomId: room.id,
            gameType: room.type,
...
function showLobby() {
    roomId = null;
    localStorage.removeItem('hst_active_room');
    $('#room').hidden = true;
    $('#lobby').hidden = false;
}
function formatGame(type) {
    return {
...
$('#joinForm').onsubmit = event => {
    ...
    joinRoom($('#joinCode').value).catch(error => showMessage(error.message, 'error'));
};
$('#ready').onclick = async () => {
    try {
        const current = (await get(ref(rtdb, `rooms/${roomId}/players/${user.uid}/ready`))).val();
        await set(ref(rtdb, `rooms/${roomId}/players/${user.uid}/ready`), !current);
    }
    catch (error) {
        showMessage(`Could not update ready status: ${error.message}`, 'error');
    }
};
$('#leave').onclick = async () => {
    try {
        await remove(ref(rtdb, `rooms/${roomId}/players/${user.uid}`));
        const host = (await get(ref(rtdb, `rooms/${roomId}/host`))).val();
        if (user.uid === host)
...
$('#matchmake').onclick = async () => {
    try {
        if (!await requirePlayer())
            return;
        showMessage('Looking for an opponent…');
        const wanted = $('#gameType').value, rounds = selectedRounds(), open = (await get(ref(rtdb, 'openRooms'))).val() || {};
        for (const [id, data] of Object.entries(open)) {
            if (data.type === wanted && Number(data.rounds || 1) === rounds) {
                try {
...
(async () => {
    ...
    if (id)
        await joinRoom(id).catch(error => showMessage(error.message, 'error'));
})();
