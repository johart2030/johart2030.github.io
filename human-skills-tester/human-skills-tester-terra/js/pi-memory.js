...
function personalBest() { return Math.max(0, Math.min(PI_DIGITS.length, HST.get('pi', 0))); }
function label(count) { return `${count} character${count === 1 ? '' : 's'}`; }
...
function showStartOptions() { form.hidden = true; answer.disabled = true; startOptions.hidden = false; continueHint.hidden = false; showBest(); }
function beginRound() {
  if (level > PI_DIGITS.length) { completeDeck(); return; }
  roundId += 1;
...
function startGame(length, mode) {
  ...
  HST.logGameEvent('game_started', { characters: level, mode });
  beginRound();
}
...
function finishGame() {
  ...
  HST.logGameEvent('game_finished', { result: 'incorrect', startCharacters: startedAt, reachedCharacters: sessionBest, targetCharacters: level });
...
continueBest.addEventListener('click', () => { const best = personalBest(); if (best > START_LENGTH && best < PI_DIGITS.length) startGame(best, 'personal_best'); });
form.addEventListener('submit', event => {
  event.preventDefault();
  const typed = answer.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');
  if (typed !== answer.value) answer.value = typed;
  if (typed !== target) { finishGame(); return; }
