...
let revealTimer = 0;
let roundId = 0;
function showBest() { const best = HST.get('pi', 0); bestEl.textContent = `${best} digit${best === 1 ? '' : 's'}`; }
...
function beginRound() {
  ...
  messageEl.textContent = `Memorize all ${level} digit${level === 1 ? '' : 's'}.`;
  form.hidden = true;
  ...
  revealTimer = setTimeout(() => {
    ...
    messageEl.textContent = `Enter the first ${level} digit${level === 1 ? '' : 's'} of π.`;
    answer.value = '';
...
function startGame() { clearTimeout(revealTimer); level = 1; beginRound(); }
function completeDeck() {
  ...
  showBest();
  messageEl.textContent = `Incredible — you recalled all ${PI_DIGITS.length} available digits in this challenge.`;
...
function finishGame() {
  ...
  answer.disabled = true;
  form.hidden = true;
  messageEl.textContent = `Close! You reached ${level - 1} digit${level === 2 ? '' : 's'}. The correct ${level}-digit sequence is shown above.`;
  startButton.hidden = false;
...
form.addEventListener('submit', event => {
  ...
  const typed = answer.value.replace(/\D/g, '');
  if (typed !== answer.value) answer.value = typed;
  if (typed === target) {
    HST.setBest('pi', level);
    showBest();
    level += 1;
    messageEl.textContent = 'Correct! Adding one more digit…';
    answer.disabled = true;
    form.hidden = true;
    revealTimer = setTimeout(beginRound, 650);
  } else finishGame();
});
answer.addEventListener('input', () => { answer.value = answer.value.replace(/\D/g, '').slice(0, level); });
showBest();
