...
onAuthStateChanged(auth, nextUser => {
    ...
    pendingLogs.splice(0).forEach(({ type, details }) => void writeLog(type, details));
});
function gameIsActive() {
    const bySelector = selector => Boolean(document.querySelector(selector));
    const checks = {
        number: () => bySelector('#start[hidden]'),
        pi: () => bySelector('#startOptions[hidden]'),
...
async function writeLog(type, details = {}) {
    ...
    try {
        ...
        console.warn('Game audit log was not saved', error);
    }
}
function recordGameEvent(event) {
    ...
    if (type === 'game_finished' && gameTrace) {
        ...
        if (Number.isFinite(resultMetric) && resultMetric >= 0)
            gameTrace.putMetric('result_value', Math.round(resultMetric));
        gameTrace.stop();
        gameTrace = null;
    }
...
(window.__hstPendingGameEvents || []).forEach(event => recordGameEvent({
    detail: event
}));
window.__hstPendingGameEvents = [];
document.addEventListener('visibilitychange', () => {
    if (!document.hidden || loggingTabSwitch || !gameIsActive())
        return;
    loggingTabSwitch = true;
    writeLog('tab_hidden').finally(() => {
        loggingTabSwitch = false;
    });
});
