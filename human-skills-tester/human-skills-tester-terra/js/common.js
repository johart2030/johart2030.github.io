const HST = {
    keys: {
        reaction: 'hst_reaction_best',
        number: 'hst_number_best',
        aim: 'hst_aim_best',
        sequence: 'hst_sequence_best',
        visual: 'hst_visual_best',
        typing: 'hst_typing_best',
        math: 'hst_math_best',
        pi: 'hst_pi_best'
    },
    ...
    setBest(key, value, lower = this.lowerIsBetter.has(key)) {
        value = Number(value);
        if (!Object.hasOwn(this.keys, key) || !Number.isFinite(value))
            return false;
        const old = this.get(key);
        if (old === null || (lower ? value < old : value > old)) {
            localStorage.setItem(this.keys[key], String(value));
            window.dispatchEvent(new CustomEvent('hst:scores-changed', {
                detail: {
                    key,
                    value
                }
    ...
    guardManualInput(element) {
        if (!element || element.dataset.manualGuarded)
            return;
        ...
        element.addEventListener('beforeinput', event => {
            if (event.inputType === 'insertFromPaste' || event.inputType === 'insertFromDrop' || event.dataTransfer)
    ...
    logGameEvent(type, details = {}) {
        ...
        window.dispatchEvent(new CustomEvent('hst:game-event', {
            detail: event
        }));
    }
};
...
function renderHomeScores() {
    ...
    if (sync)
        sync.textContent = window.hstUser ? (document.documentElement.dataset.scoreSync === 'ready' ? 'Cloud scores synced' : 'Syncing scores…') : 'Scores saved on this device';
}
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-reset]').forEach(button => button.addEventListener('click', () => {
        if (confirm('Reset all saved scores on this device and in your account?'))
            window.dispatchEvent(new Event('hst:reset-requested'));
    }));
